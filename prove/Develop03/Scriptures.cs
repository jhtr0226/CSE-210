...
public class Scripture
{
    ...
    public void HideLastThreeWords()
    {
        ...
        for (int indexNumber = 0; indexNumber < 3; indexNumber++)
        {
            ...
            } while (_hidingIndices.Contains(randomIndexNumber)); //trying not to repeat it
    ...
    public void hiding(List<int> alreadyHiddenIndices)
    {
        List<int> availableIndices = _words
            ...
            //filters non hidden words from hidden ones (done this by research (took me like 3 days) and found out it loks like javascript)
            .Select(item => item.Index) //selects indices
            .ToList(); //converts to a list


        /*that last part was a miracle, a friend told me to uses those .select, .where and .ToList
        that blew my mind, I should study them more*/
    ...
    {
        return _words.TrueForAll(word => word.IsHidden()); //makes sure everything is hidden
    }
